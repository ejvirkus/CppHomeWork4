...
Homework::Tasks Homework::assignment2()
{
    ...
    results.push_back({"number_set is a set", (std::is_same_v<decltype(set), std::set<int>>)});
    results.push_back({"number_set has 3 elements", (set.size() == 3)});
    results.push_back({"number_set has 1, 2, 3", (set.find(1) != set.end()) && (set.find(2) != set.end()) && (set.find(3) != set.end())});
